...
def insert_nonzero_rational_number(editor):
    """Generate Anki card for non-zero rational number (Q*) check."""
    ...
    front_text = f"<b>Números Racionais Não Nulos</b><br>Diga se o número {number} é racional não nulo (Q*)"
    ...
    if is_rational_number(number):
        ...
        back_text = (f"<b>Desenvolvimento</b><br>"
                     f"Os números racionais não nulos são os números que podem ser escritos na forma de fração, excluindo o zero. Esses números podem também ter representação decimal finita ou decimal infinita e periódica, excluindo o zero.<br>"
                     f"Q* = {{a/b | aEZ, bEZ* e Q ≠ 0}} <br>"
                     f"Q* = {{ \"a\" sobre \"b\", tal que, \"a\" pertence aos números inteiros e \"b\" pertence aos números inteiros não nulos (menos o zero), e \"Q\" ≠ 0 }} <br><br>"
                     f"Sim, {number} é um número racional não nulo (Q*)<br>Q* = {rational_set}")
    else:
        back_text = (f"<b>Desenvolvimento</b><br>"
                     f"Os números racionais não nulos são os números que podem ser escritos na forma de fração, excluindo o zero. Esses números podem também ter representação decimal finita ou decimal infinita e periódica, excluindo o zero.<br>"
                     f"Q* = {{a/b | aEZ, bEZ* e Q ≠ 0}} <br>"
                     f"Q* = {{ \"a\" sobre \"b\", tal que, \"a\" pertence aos números inteiros e \"b\" pertence aos números inteiros não nulos (menos o zero), e \"Q\" ≠ 0 }} <br><br>"
                     f"Não, {number} não é um número racional não nulo (Q*)<br>Q* = {{...}}")
