...
def insert_nonpositive_integer_number(editor):
    """Generate Anki card for nonpositive integer number (Z-) check."""
    
    def is_integer_number(n):
        """Check if a number is a nonpositive integer number (Z-)."""
        return isinstance(n, int) and n <= 0
    
    def generate_nonpositive_integer_set(n):
        """Generate the set of nonpositive integers from n to 0."""
    ...
    # Check if the number is a nonpositive integer
    is_nonpositive_integer = is_integer_number(number)
    
    # Montando a expressão para a frente da carta
    front_text = f"<b>Numeros Inteiros Não Positivos</b><br>Diga se o número {number} é inteiro não positivo (Z-)"
    
    # Montando o texto para o verso da carta
    if is_nonpositive_integer:
        nonpositive_integer_set = generate_nonpositive_integer_set(int(number))
        nonpositive_integer_set_str = ", ".join(map(str, sorted(nonpositive_integer_set)))
        back_text = f"<b>Desenvolvimento</b><br>Sim, {number} é um número inteiro não positivo (Z-)<br>Z- = {{{nonpositive_integer_set_str}}}"
    else:
        back_text = f"<b>Desenvolvimento</b><br>Não, {number} não é um número inteiro não positivo (Z-)"
