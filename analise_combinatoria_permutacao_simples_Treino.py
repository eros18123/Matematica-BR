...
score_file = os.path.join(addon_path, "scores_permutacao_simples.json")
last_training_file = os.path.join(addon_path, "last_training_permutacao_simples.json")
...
class FinalScoreDialog(QDialog):
    def __init__(self, correct_answers, incorrect_answers, score, total_correct_answers, total_incorrect_answers, total_score, parent=None):
        ...
        if correct_answers <= 2:
            message = "Péssimo: Você não está apto nesta matéria, treine mais"
        elif correct_answers <= 5:
            message = "Razoável: Você ainda não está bem, precisa melhorar. ⭐"
...
class Permutacao_Simples_Treino(QDialog):
    ...
    def generate_questions(self):
        ...
        for i in range(1, 11):
            ...
            correct_answer = math.factorial(n)
            wrong_answers = set()
            ...
            question_text = f"<b>Questão {i}: Quantas permutações existem de {n} elementos?</b>"
